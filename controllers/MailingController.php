...
class MailingController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if($request->isAjax){
            ...
            if($model->load($request->post()) && $model->validate()){
                if(($post['yur'] === '1' || $post['fiz'] === '1') && ($post['email'] === '1' || $post['sms'] === '1') ) {
                    $model->sendSms($post);
                    if($post['yur'] === '1') $model->user .= 'Юр. лицо;';
                    if($post['fiz'] === '1') $model->user .= 'Физ. лицо;';
                    ...
                    return [
                        'forceReload'=>'#crud-datatable-pjax',
                        'title'=> "Рассылка",
                        'content'=>'<span class="text-success">Выберите (Юр. лицо или Физ. лицо) или (E-mail или SMS)</span>',
                        'footer'=> Html::button('Отмена',['class'=>'btn btn-default pull-left','data-dismiss'=>"modal"]).
                                Html::a('Создать ещё',['create'],['class'=>'btn btn-primary','role'=>'modal-remote'])
                    ];
                }
    ...
    /**
     * Delete an existing Mailing model.
     * For ajax request will return json object
    ...
    public function actionDelete($id)
    {
     ...
     /**
     * Delete multiple existing Mailing model.
     * For ajax request will return json object
     * and for non-ajax request if deletion is successful, the browser will be redirected to the 'index' page.
     * @param integer $id
     * @return mixed
